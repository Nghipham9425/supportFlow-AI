...
public class KnowledgeEmbeddingService : IKnowledgeEmbeddingService
{
    ...
    public KnowledgeEmbeddingService(AppDbContext dbContext, IEmbeddingProvider embeddingProvider)
    {
    ...
    public async Task<List<KnowledgeChunkDto>?> GenerateEmbeddingsAsync(Guid articleId)
    {
        ...
        {
            await _embeddingProvider.GenerateEmbeddingAsync(chunk.Content);
